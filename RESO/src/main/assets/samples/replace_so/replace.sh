...
replace() {
    ...
    if [ $? != 0 ]; then
        mv /data/app/com.tencent.tmgp.sgamece*/lib/arm64/libtersafe.so /data/local/tmp/lib/armeabi-v7a/libtersafe.so.bak 2> /dev/null && echo "备份成功, 备份路径为: /data/local/tmp/lib/armeabi-v7a/libtersafe.so.bak" || echo "备份失败, 请使用“替换(DEBUG)”重试 并 联系作者: 酷安 BY 山抹清风"
    else
